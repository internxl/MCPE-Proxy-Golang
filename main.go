...
var Address = "play.minenex.fun:19132"
var Nick = "player"
var Input = 2
var Os = 1
var Device = "XIAOMI SS23224242MT"
var CurrentCid = -rand.Int63()

func main() {
	//_, _ = strconv.Atoi(strings.Split(LocalAddress, ":")[0])
	log.Println("bind port set to", CurrentBindedPlayerPort)
		...
		case <-ticker.C:
			...
			if pks := ClientCon.DecodeBatch(buffer[1:size]); pks != nil {
				...
				for pos, pk := range pks {
					...
					switch p := pkt.(type) {
                    case *dataPacket.TextPacket:
                    
					   if p.Message == ".ss" {
						   pks = append(pks[:pos], pks[pos+1:]...)
						   out := ""
						   for uuid, sd := range Entries {
							   out += "saved skin for " + uuid + "\n"
							   utils.Skin2png(sd, "skins/"+uuid+".png")
							}
							if out == "" {
								out = "no skins saved"
							}
							Entries = map[string]string{}
							ClientCon.SendPacket(&dataPacket.TextPacket{
								SourceName: out,
								TextType:    0,
							})
						}
		...
		case <-ticker.C:
			...
			pkloop:
				for _, pk := range pks {
					...
					//pks = append(pks[:pos], pks[pos+1:]...)
					//continue
					case *dataPacket.PlayerListPacket:
					   if p.Type == 0 {
						   for uuid, ent := range p.Entries {
							   Entries[uuid] = ent[3].(string)
							   log.Println("saved skin for", uuid)
